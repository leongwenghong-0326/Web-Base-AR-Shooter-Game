...
import { GameAudio } from './audio.js?v=1789445200';
import { Effects } from './effects.js?v=1789445200';
import { Weapon } from './weapon.js?v=1789445200';
import { EnemyFactory } from './enemy.js?v=1789445200';
import { GameManager } from './game.js?v=1789445200';

/** Lazy-loaded — MindAR bundle is ~2MB and was blocking page ready. */
...
function withTimeout(promise, ms, label) {
  ...
  const timeout = new Promise((_, reject) => {
    timer = setTimeout(() => reject(new Error(label + ' timed out (' + (ms / 1000) + 's). Check Wi‑Fi / tunnel.')), ms);
  });
...
async function startAR() {
  ...
  if (/FBAN|FBAV|Instagram|Line\/|WhatsApp|MicroMessenger|TikTok/i.test(ua)) {
    throw new Error('Open in Chrome/Safari: tap ··· → Open in browser. In-app browsers block the camera.');
  }
