...
export class Weapon {
  constructor({ camera, scene, audio, effects, onAmmoChange }) {
    ...
    // Lower + farther so the gun stays clearly below the crosshair
  ...
  async load(url = 'assets/models/fps-akm.glb') {
    ...
    const scale = IS_ANDROID ? 0.28 : IS_MOBILE ? 0.26 : 0.22;
  ...
  syncToCamera() {
    ...
    const z = -Math.max(Math.abs(this.localOffset.z), near + 0.85);
